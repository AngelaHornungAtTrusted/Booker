...
function booker_event_meta_box()
{
    ...
    <div class="row">
        <div class="col-md-4">
            <input type="hidden" name="booker_meta_box_nonce"
...
function save_your_fields_meta($post_id)
{
    if (wp_verify_nonce($_POST['booker_meta_box_nonce'], basename(__FILE__))) {
        if (current_user_can('edit_post', $post_id) || current_user_can('edit_page', $post_id)) { //delete_post_meta($post_id, 'booker-event-meta-date')
            ((isset($_POST['booker-event-meta-date'])) ? update_post_meta($post_id, 'booker-event-meta-date', $_POST['booker-event-meta-date']) : '');
            ((!isset($_POST['booker-event-meta-registration'])) ? update_post_meta($post_id, 'booker-event-meta-registration', 0) : update_post_meta($post_id, 'booker-event-meta-registration', 1));
        }
    }
}
...
function booker_append_registration($content)
{
    if (is_singular('event') && in_the_loop() && is_main_query()) {
        if (intval(get_post_meta(intval(get_the_ID()), 'booker-event-meta-registration', true)) > 0) {
            ...
            $content . include(plugin_dir_path(__FILE__) . 'registration/non-registration.php');
        }
    }
